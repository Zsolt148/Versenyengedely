<x-jet-action-section>
    <x-slot name="title">
        Böngésző munkamenetek
    </x-slot>

    <x-slot name="description">
        {{ __('Jelentkezz ki más böngészőkből időnként') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600 dark:text-white">
            {{ __('Ha szükséges, kijelentkezhetsz minden más munkamenetből. Néhány munkameneted látható lentebb, ugyanakkor ez a lista nem mindig pontos.') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500 dark:text-gray-300">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('Ez az eszköz') }}</span>
                                    @else
                                        {{ __('Legutoljára aktív') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('Kijelentkezés minden munkamenetből') }}
            </x-jet-button>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            <x-slot name="title">
                {{ __('Kijelentkezés minden más munkamenetből') }}
            </x-slot>

            <x-slot name="content">
                {{ __('Kérlek írd be a jelszavad hogy meggyőződhessünk hogy te szeretnél kijelentkezni a munkamenetből.') }}

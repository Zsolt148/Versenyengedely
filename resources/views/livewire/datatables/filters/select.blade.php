<div x-data class="flex flex-col">
    <div class="flex">
        <select
            x-ref="select"
            name="{{ $name }}"
            class="m-1 text-sm leading-4 flex-grow form-select rounded-md border-gray-400 dark:bg-gray-700"
    ...
    <div class="flex flex-wrap max-w-48 space-x-1">
        ...
        <button wire:click="removeSelectFilter('{{ $index }}', '{{ $key }}')" x-on:click="$refs.select.value=''"
            class="m-1 pl-1 flex items-center uppercase tracking-wide bg-gray-300 dark:bg-gray-500 text-white hover:bg-red-600 rounded-full focus:outline-none text-xs space-x-1">
